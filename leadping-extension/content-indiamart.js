(function() {
  ...
  function init() {
    StorageUtil.getSettings().then(function(settings) {
      if (!settings.platforms || settings.platforms.indiamart === false) return;
      ...
      StorageUtil.get('device_id').then(function(deviceId) {
        ...
        StorageUtil.get('user_id').then(function(userId) {
          if (userId) {
            ...
            StorageUtil.get('tier').then(function(t) { currentTier = t || 'free'; });
            startScan();
          } else {
    ...
    try {
      var observer = new MutationObserver(function() {
        try {
          ...
          scanDebounceTimer = setTimeout(scanForLeads, 300);
        } catch(e) {}
      });
  ...
  function scanForLeads() {
    try {
      ...
      cards.forEach(function(card) {
        try {
          if (!card.hasAttribute('data-lp-processed')) {
            processLeadCard(card);
          }
        } catch(e) {}
      });
    } catch(e) {}
  ...
  function processLeadCard(card) {
    try {
      ...
      var leadId = (card.dataset && card.dataset.leadId) ? card.dataset.leadId : ('lp_' + Date.now() + '_' + Math.random().toString(36).slice(2, 8));
      var lead = Object.assign({ lead_id: leadId, detected_at: Date.now(), status: 'pending' }, leadData);
      StorageUtil.saveLead(lead);
      if (currentUserId) {
        ApiClient.saveLead(currentUserId, lead);
      }
  ...
  function safeText(el) {
    try { return el ? el.innerText.trim() : null; } catch(e) { return null; }
  }
  ...
  function extractLeadData(card) {
    var buyer_name = trySelectors(card, [
      '[class*="buyer-name"]', '[class*="buyerName"]', '[class*="BuyerName"]', '[class*="name"]', 'h3', 'h4', 'h2'
    ]);

    var buyer_phone = trySelectors(card, [
      '[class*="mobile"]', '[class*="phone"]', '[data-phone]', '[class*="contact"]'
    ]);
    if (!buyer_phone) {
      try { var m = card.innerText.match(/[6-9]\d{9}/); buyer_phone = m ? m[0] : null; } catch(e) {}
    }

    var product = trySelectors(card, [
      '[class*="product"]', '[class*="inquiry"]', '[class*="subject"]', 'p'
    ]);

    var city = trySelectors(card, [
      '[class*="location"]', '[class*="city"]', '[class*="address"]', '[class*="place"]'
    ]);

    var company = trySelectors(card, [
      '[class*="company"]', '[class*="firm"]', '[class*="org"]'
    ]);

    var buyer_email = null;
    try { var em = card.innerText.match(/[\w.-]+@[\w.-]+\.\w+/); buyer_email = em ? em[0] : null; } catch(e) {}

    return { buyer_name: buyer_name, buyer_phone: buyer_phone, buyer_email: buyer_email, product: product, city: city, company: company, platform: 'indiamart' };
  }
  ...
  function injectLeadPingUI(card, lead) {
    try {
      ...
      container.setAttribute('data-lp-id', lead.lead_id);
  ...
  function handleWAClick(btn, lead) {
    ...
    ApiClient.checkUsage(currentUserId, 'whatsapp').then(function(usage) {
      if (usage && usage.allowed === false) {
        showUpgradeModal();
        btn.disabled = false;
        btn.textContent = '📱 WhatsApp Follow-Up';
        return;
      }
      generateAndShowModal(lead, function() {
        btn.disabled = false;
        btn.textContent = '📱 WhatsApp Follow-Up';
      });
    }).catch(function() {
  ...
  function showMessageModal(lead, message) {
    try {
      ...
      title.className = 'lp-modal-title';
      title.textContent = '📱 Send WhatsApp Message';

      var closeBtn = document.createElement('button');
      closeBtn.className = 'lp-modal-close';
      ...
      var buyerInfo = document.createElement('p');
      buyerInfo.className = 'lp-modal-buyer';
      ...
      labelRow.className = 'lp-msg-label';
      var lbl = document.createElement('label');
      lbl.textContent = '✨ AI generated message (editable)';
      var regenBtn = document.createElement('button');
      regenBtn.className = 'lp-regen-btn';
      regenBtn.textContent = '🔄 Regenerate';
      labelRow.appendChild(lbl);
      labelRow.appendChild(regenBtn);

      var textarea = document.createElement('textarea');
      textarea.className = 'lp-msg-textarea';
      ...
      cancelBtn.className = 'lp-btn-cancel';
      cancelBtn.textContent = 'Cancel';
      cancelBtn.addEventListener('click', function() { document.body.removeChild(overlay); });

      var sendBtn = document.createElement('button');
      sendBtn.className = 'lp-btn-send';
      ...
      sendBtn.addEventListener('click', function() {
        ...
        StorageUtil.getSettings().then(function(settings) {
          if (settings.reminder_enabled) {
            chrome.runtime.sendMessage({ type: 'SCHEDULE_REMINDER', lead_id: lead.lead_id, hours: settings.reminder_interval_hours || 24 }).catch(function(){});
          }
        });
      ...
      regenBtn.addEventListener('click', function() {
        ...
        chrome.runtime.sendMessage({
          ...
          textarea.value = (response && response.message) ? response.message : message;
  ...
  function showUpgradeModal() {
    try {
      ...
      var modal = document.createElement('div');
      modal.className = 'lp-upgrade-modal';

      modal.innerHTML = '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px">' +
        '<h3 style="margin:0;font-size:17px;font-weight:600;color:#0C2D5E">🚀 Upgrade to LeadPing Pro</h3>' +
        '<button onclick="this.closest(\'.lp-modal-overlay\').remove()" style="background:none;border:none;font-size:18px;cursor:pointer;color:#6B7280">✕</button>' +
        '</div>' +
        '<div class="lp-plan-row">' +
        '<div class="lp-plan-card"><div class="plan-name">Free</div><div class="plan-price">₹0</div><div class="plan-feature">10 WA/day<br>3 AI/day<br>IndiaMART only</div></div>' +
        '<div class="lp-plan-card highlight"><div class="plan-name">Pro</div><div class="plan-price">₹999/mo</div><div class="plan-feature">Unlimited WA<br>Unlimited AI<br>All 3 platforms</div></div>' +
        '<div class="lp-plan-card"><div class="plan-name">Lifetime</div><div class="plan-price">₹1,999</div><div class="plan-feature">Unlimited WA<br>Unlimited AI<br>Forever ✨</div></div>' +
        '</div>' +
        '<div class="lp-upi-box">💳 Pay ₹1,999 to <strong>leadping@upi</strong> and send screenshot on WhatsApp: <strong>+91 9999999999</strong></div>' +
        '<a class="lp-activate-link" id="lp-go-options">Already bought? Activate your license key →</a>';

      overlay.appendChild(modal);
      overlay.addEventListener('click', function(e) { if (e.target === overlay) overlay.remove(); });
      document.body.appendChild(overlay);

      var goOpts = document.getElementById('lp-go-options');
      if (goOpts) {
        goOpts.addEventListener('click', function() {
          chrome.runtime.sendMessage({ type: 'OPEN_OPTIONS' }).catch(function(){});
          overlay.remove();
        });
      }
    } catch(e) {}
