(function() {
  ...
  function init() {
    StorageUtil.getSettings().then(function(settings) {
      if (!settings.platforms || settings.platforms.justdial === false) return;
      ...
      StorageUtil.get('device_id').then(function(deviceId) {
        ...
        StorageUtil.get('user_id').then(function(userId) {
          if (userId) {
            ...
            StorageUtil.get('tier').then(function(t) {
              currentTier = t || 'free';
              checkTierAndStart();
            });
          } else {
  ...
  function injectUpgradeBanner() {
    try {
      if (document.getElementById('lp-upgrade-jd')) return;
      var banner = document.createElement('div');
      banner.id = 'lp-upgrade-jd';
      banner.className = 'lp-upgrade-banner';
      banner.innerHTML = '⚡ <strong>LeadPing</strong>: Upgrade to LeadPing Pro to enable JustDial support. ' +
        '<a href="#" id="lp-jd-upgrade-link" style="color:#185FA5;font-weight:500">Upgrade now →</a>';
      var target = document.querySelector('body > *:first-child') || document.body;
      document.body.insertBefore(banner, target);
      var link = document.getElementById('lp-jd-upgrade-link');
  ...
  function scanForLeads() {
    try {
      var selectors = ['.lead-row', '[class*="lead-item"]', 'tr[data-id]', '.enquiry-row', '[class*="enquiry"]'];
      var cards = [];
  ...
  function extractLeadData(card) {
    var buyer_name = trySelectors(card, ['[class*="cust-name"]', '[class*="custName"]', '[class*="customer"]', 'td:first-child']);
    var buyer_phone = trySelectors(card, ['[class*="mob"]', '[class*="phone"]', 'td[class*="mobile"]', '[class*="contact"]']);
    if (!buyer_phone) { try { var m = card.innerText.match(/[6-9]\d{9}/); buyer_phone = m ? m[0] : null; } catch(e) {} }
    var product = trySelectors(card, ['[class*="service"]', '[class*="category"]', '[class*="catg"]', '[class*="keyword"]']);
    var city = trySelectors(card, ['[class*="city"]', '[class*="area"]', '[class*="location"]', '[class*="locality"]']);
    var company = trySelectors(card, ['[class*="company"]', '[class*="org"]']);
    var buyer_email = null;
    try { var em = card.innerText.match(/[\w.-]+@[\w.-]+\.\w+/); buyer_email = em ? em[0] : null; } catch(e) {}
    return { buyer_name: buyer_name, buyer_phone: buyer_phone, buyer_email: buyer_email, product: product, city: city, company: company, platform: 'justdial' };
  }
  ...
  function handleWAClick(btn, lead) {
    ...
    ApiClient.checkUsage(currentUserId, 'whatsapp').then(function(usage) {
      if (usage && usage.allowed === false) {
        showUpgradeModal();
        btn.disabled = false; btn.textContent = '📱 WhatsApp Follow-Up'; return;
      }
  ...
  function generateAndShowModal(lead, onDone) {
    chrome.runtime.sendMessage({
      ...
      try {
        var message = (response && response.message) ? response.message : 'Hello! JustDial pe aapki query mili. Kab baat ho sakti hai? 📞';
  ...
  function showMessageModal(lead, message) {
    try {
      ...
      cancelBtn.className = 'lp-btn-cancel'; cancelBtn.textContent = 'Cancel';
      cancelBtn.addEventListener('click', function() { overlay.remove(); });

      var sendBtn = document.createElement('button');
      sendBtn.className = 'lp-btn-send'; sendBtn.textContent = 'Send on WhatsApp ✓';
      ...
      regenBtn.addEventListener('click', function() {
        ...
        chrome.runtime.sendMessage({ type: 'GENERATE_AI_MESSAGE', user_id: currentUserId, buyer_name: lead.buyer_name||'', product: lead.product||'', city: lead.city||'', company: lead.company||'', platform: 'justdial' }, function(r) {
  ...
  function showUpgradeModal() {
    try {
      ...
      modal.innerHTML = '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px"><h3 style="margin:0;font-size:17px;font-weight:600;color:#0C2D5E">🚀 Upgrade to LeadPing Pro</h3><button onclick="this.closest(\'.lp-modal-overlay\').remove()" style="background:none;border:none;font-size:18px;cursor:pointer">✕</button></div>' +
        ...
        '<a href="#" id="lp-jd-opts" class="lp-activate-link">Already bought? Activate your license key →</a>';
      overlay.appendChild(modal);
      overlay.addEventListener('click', function(e) { if (e.target === overlay) overlay.remove(); });
      document.body.appendChild(overlay);
      var link = document.getElementById('lp-jd-opts');
